//
//  1-to-1ConversationViewController.m
//  SafeChat
//
...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *leftCloseItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"btn_back"] style:UIBarButtonItemStyleDone target:self action:@selector(close)];
    ...
    [rightOpenMoreMenuItem setTintColor:[UIColor colorWithRed:42.0/255.0 green:172.0/255.0 blue:77.0/255.0 alpha:1.0]];
    ;
    ...
    if (sender == self.channel) {
        if (sender.getTypingMembers.count == 0) {
            ...
            if (sender.getTypingMembers.count == 1) {
                [self.chattingView startTypingIndicator:[NSString stringWithFormat:@"%@ started typing...", sender.getTypingMembers[0].nickname]];
            }
            else {
                [self.chattingView startTypingIndicator:@"Members starting typing..."];
            }
        }
    }
}
        ...
        if ([baseMessage isKindOfClass:[SBDUserMessage class]]) {
            ...
            if (userMessage.customType != nil && [userMessage.customType isEqualToString:@"url_preview"]) {
                ...
                if ([sender.userId isEqualToString:[SBDMain getCurrentUser].userId] && self.chattingView.preSendMessages[((SBDUserMessage *)baseMessage).requestId] == nil) {
                    deleteMessageAction = [UIAlertAction actionWithTitle:@"Delete message" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
...
- (void)clickDelete:(UIView *)view message:(SBDBaseMessage *)message {
    UIAlertController *vc = [UIAlertController alertControllerWithTitle:@"Delete message failed" message:@"Deleting the message failed" preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *closeAction = [UIAlertAction actionWithTitle:@"Close" style:UIAlertActionStyleCancel handler:nil];
    UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:@"Delete message" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        OneToOneConversationViewController *strongSelf = weakSelf;
